...
From C<v0.2.0>  all functions creating a new fd behave more perlish and apply
the CLOEXEC if larger than C<$^F>  to prevent fd leakage.  This may result in
an extra system call you didn't need if your program never calls C<exec>. To
disable this, increase the value of C<$^F>.
...
From version B<v0.2.0>.  Implements the linux  C<accept4> syscall. On non linux
systems this is emulated by calling C<fcntl> to set the FD_CLOEXEC flag and
O_NONBLOCK status. Returns C<undef> on error, for the user to test C<$!>. 

The flags argument can be the bitwise or'd value of C<IO::FD::SOCK_NONBLOCK>,
C<IO::FD::SOCK_CLOEXEC>.

Unlike other functions returning new file descriptors, this B<DOES NOT>
...
call is used to set the SOCK_NONBLOCK flag.
