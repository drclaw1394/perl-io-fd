...
=head2 v0.2.0
All functions creating a new fd behave more perlish and apply the CLOEXEC if
...
increase the value of C<$^F>.

Exceptions are now thrown when output variables (fds) are read only. This
matches perl behaviour.

When the input fd variable doesn't look like an fd, warnings are generated,
